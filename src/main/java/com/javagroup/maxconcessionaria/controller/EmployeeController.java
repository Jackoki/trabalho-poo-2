...
public class EmployeeController {
    ...
    public void validateEmployee(Employee employee) {
        if (isNullOrEmpty(employee.getName()) || employee.getName() == null) {
    ...
    public void save(Employee employee) {
        try{
            ...
            jdbcTemplate.update(sqlUser, employee.getCategory(), employee.getName(), employee.getAddress(), employee.getEmail(), employee.getPhone(), employee.getPassword());
            
            String checkSql = "SELECT id FROM User WHERE name = ?";
            Integer generatedUserId = jdbcTemplate.queryForObject(checkSql, new Object[]{employee.getName()}, Integer.class);
            
            String sqlCustomer = "INSERT INTO Employee (idUser, lvlPermission) VALUES (?, ?)";
            jdbcTemplate.update(sqlCustomer, generatedUserId, employee.getLvlPermission());
    ...
    public boolean loginEmployee(String name, String password) throws UserException {
        try {
            String checkSql = "SELECT COUNT(*) FROM User u INNER JOIN Employee e ON (u.id = e.idUser) WHERE u.name = ? AND u.password = ?";
            ...
            JOptionPane.showMessageDialog(null, "ERRO NA AÇÃO DO BANCO DE DADOS!", "LOGIN DE FUNCIONÁRIO", 0);
            return false;
        }
    ...
    public void update(String name, String passwordChecker, String password, String address, String email, String phone) throws WrongDataException {
        try{
            ...
            JOptionPane.showMessageDialog(null, "USUÁRIO ATUALIZADO COM SUCESSO!", "ATUALIZAÇÃO DE FUNCIONÁRIO", 1);
        }
       
        catch (DataAccessException ex){
            JOptionPane.showMessageDialog(null, "ERRO NA AÇÃO DO BANCO DE DADOS!", "ERRO NA ATUALIZAÇÃO DE FUNCIONÁRIO", 0);
        }
        
        catch (NumberFormatException nfe){
            JOptionPane.showMessageDialog(null, "TIPO DE DADO CADASTRADO ERRADO OU VAZIO!", "ERRO NA ATUALIZAÇÃO DE FUNCIONÁRIO", 0);
        }
        
        catch (IllegalArgumentException e) {
            JOptionPane.showMessageDialog(null, "TIPO DE DADO CADASTRADO ERRADO!", "ERRO NA ATUALIZAÇÃO DE FUNCIONÁRIO", 0);
        }
        
        catch (WrongDataException ndne) {
            ndne.screenException();
        }
    }
